...
BEGIN
    -- 로그인
    SELECT password, acc_id INTO v_password, p_acc_id
    FROM nf_account
    WHERE email = p_email;
...
EXEC ins_signin('bcde@naver.com', 'b12345');
SELECT * FROM streaming_dev;
SELECT * FROM nf_account;
DELETE FROM streaming_dev WHERE mac_id = '94-92-94-F8-99-14';COMMIT;
